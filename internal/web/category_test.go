...
func TestSavingANearDuplicateCategoryWithoutScriptsAsksAboutIt(t *testing.T) {
	ctx, store, h, _, _ := typoFixture(t)
	list := packing.NewList("user", "River", "")
	list.Items = []packing.PackingItem{packing.NewItem("Bait", "Fishing")}
	if err := store.SavePackingList(ctx, list); err != nil {
		t.Fatal(err)
	}
	route := func(r *http.Request) *http.Request {
		rc := chi.NewRouteContext()
		rc.URLParams.Add("id", list.ID)
		return r.WithContext(context.WithValue(r.Context(), chi.RouteCtxKey, rc))
	}
	add := func(category string, htmx bool) *httptest.ResponseRecorder {
		t.Helper()
		r := packingRequest("/packing-lists/"+list.ID+"/add-item", url.Values{"name": {"Lures"}, "category": {category}})
		if htmx {
			r.Header.Set("HX-Request", "true")
		}
		w := httptest.NewRecorder()
		h.AddItemHandler(w, route(r))
		return w
	}
	...
	lures := saved.Items[len(saved.Items)-1]
	if w.Code != 303 || lures.Category != "Fihsing" || w.Header().Get("Location") != "/packing-lists/"+list.ID+"?check="+lures.ID {
		t.Fatalf("near duplicate saved as %q: %d %v", lures.Category, w.Code, w.Header())
	}
	r := route(httptest.NewRequest("GET", w.Header().Get("Location"), nil))
	r = r.WithContext(context.WithValue(r.Context(), auth.USER_ID_KEY, "user"))
	page := httptest.NewRecorder()
	h.ListDetailsPage(page, r)
	for _, want := range []string{"Did you mean “Fishing”?", `action="/packing-lists/` + list.ID + `/edit-item/` + lures.ID + `"`, `name="category" value="Fishing"`, `name="forget" value="Fihsing"`, "Keep “Fihsing”"} {
		if !strings.Contains(page.Body.String(), want) {
			t.Errorf("list page missing %q", want)
		}
	}

	// Using the suggestion files the item under it and forgets the typo.
	r = withItemRoute(packingRequest("/packing-lists/"+list.ID+"/edit-item/"+lures.ID, url.Values{"name": {"Lures"}, "category": {"Fishing"}, "forget": {"Fihsing"}, "revision": {saved.Revision()}}), list.ID, lures.ID)
	edit := httptest.NewRecorder()
	h.EditItemHandler(edit, r)
	if edit.Code != 303 || edit.Header().Get("Location") != "/packing-lists/"+list.ID {
		t.Fatalf("use suggestion: %d %v %s", edit.Code, edit.Header(), edit.Body.String())
	}
	if got := itemCategoryNames(t, store, list.ID, "user"); got[len(got)-1] != "Fishing" {
		t.Errorf("used suggestion saved %q", got)
	}
	if got, err := store.RememberedCategories(ctx, "user"); err != nil || slices.Contains(got, "Fihsing") {
		t.Errorf("typo still remembered: %q, %v", got, err)
	}

	for _, test := range []struct {
		category string
		htmx     bool
	}{{"fishing", false}, {"Kayaking", false}, {"Fishign", true}} {
		w := add(test.category, test.htmx)
		if strings.Contains(w.Header().Get("Location"), "check=") {
			t.Errorf("%q asked about a close category: %v", test.category, w.Header())
		}
	}
}
