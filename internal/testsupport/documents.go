...
func (m *Documents) NewQueryItemsPager(query string, _ azcosmos.PartitionKey, options *azcosmos.QueryOptions) *runtime.Pager[azcosmos.QueryItemsResponse] {
...
func (m *Documents) DeleteItem(_ context.Context, _ azcosmos.PartitionKey, id string, _ *azcosmos.ItemOptions) (azcosmos.ItemResponse, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if _, ok := m.docs[id]; !ok {
		return azcosmos.ItemResponse{}, &azcore.ResponseError{StatusCode: 404}
