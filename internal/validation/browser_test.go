...
func TestBrowserSimulation(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if offline.Load() && !strings.HasPrefix(r.URL.Path, "/__validation/") {
	...
	server = &http.Server{Addr: "127.0.0.1:3001", Handler: web.CSRFProtection([]byte("camplist-validation-csrf-key-32!!"), false, "camplist-validation.localhost:3001")(handler)}
