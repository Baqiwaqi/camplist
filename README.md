...
3. Add items, such as a tent or sleeping bag, with an optional category. Pick one of the basic camping categories or type your own; Camplist remembers new categories and offers them on your other lists. While you type, the picker suggests a close existing category first. Fix a mistyped category with Rename (it updates items on lists you own, not trips already started) or drop it from your suggestions with Remove, from the picker or the Categories page.
   To add many at once, use **Add several** to type or paste one item per line (a line ending in a colon, like `Documents:`, starts a category, and you can pick a category for the lines above the first heading), or **Add from a list** to copy the items you tick from another list, such as a small `Climbing` or `Documents` list. Add several takes up to 100 lines at a time; a copy is limited only by the list's size. Names already on the list, or repeated in the same add, are skipped and reported, and copied items do not change when the other list does.
...
The Go server renders HTML using templ. HTMX adds form navigation and actions such as deleting records and checking items. Packing check-off swaps only the toggled row and its progress, group and tab counts, and trip renames and additions update the page in place; archiving, restoring or deleting a trip removes its card in place (from the trip page it returns to Trips), and other deletions reload or leave the page. Normal packing forms also work without HTMX.
...
| `static/` | Built stylesheet, logo marks, the dropdown menu, select, category picker and tab behaviour, and the bundled htmx and Alpine.js scripts |
...
choose **Save and apply now**. While the list is available, each observation
shows whether its change is waiting or applied. Old sessions keep their original checklist.
